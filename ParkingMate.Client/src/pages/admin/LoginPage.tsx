...
export function LoginPage() {
  ...
  const loginMutation = useAdminLogin((data) => {
    if (data.success && data.bearerToken) {
      console.log('🔑 Admin login response:', {
        email: data.email,
        sessionId: data.sessionId,
        roles: data.roles,
        bearerToken: data.bearerToken?.substring(0, 20) + '...'
      })
      
      // Clear any old session data first to avoid stale roles
      sessionStorage.removeItem('customer-session')
      
      // Store tokens in Zustand session store (this is what RouteGuard checks!)
      // Use roles from ServiceStack login response
      setTokens(data.bearerToken, undefined, {
        email: data.email || '', // Email from login response
        sessionId: data.sessionId,
        roles: (data.roles as ('Admin' | 'Customer')[]) || [] // Roles from ServiceStack response
      })
