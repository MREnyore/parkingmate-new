...
import { getJWTTimeToExpiry, isJWTExpired, isServiceStackSession } from './jwt'
...
class TokenExpirySubscriber {
  ...
  private async checkTokenExpiry() {
    ...
    // Skip ALL validation for ServiceStack sessions (admin sessions)
    // ServiceStack manages session validation via cookies, not JWT
    if (isServiceStackSession(accessToken)) {
      console.log('ServiceStack session detected, skipping all token validation (managed by ServiceStack)')
      // No validation needed - ServiceStack handles session expiry via cookies
      return
    }
