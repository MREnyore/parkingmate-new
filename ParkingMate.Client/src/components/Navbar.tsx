...
export function Navbar() {
  ...
  const { data: customerInfo, isLoading: isLoadingCustomer } =
    useGetCustomerInfo()

  // Fetch admin info ONLY for admins
  const { data: adminInfo, isLoading: isLoadingAdmin } = useAdminUserInfo()
  ...
  const formatDateTime = () => {
    ...
    return currentTime.toLocaleString('de-DE', options).replace(',', ' -')
  }
    ...
    <header className="flex py-4 px-12 shrink-0 items-center justify-between gap-2 border-b bg-white h-[100px]">
      <div className="flex items-center gap-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-md text-gray-600">{formatDateTime()}</span>
        </div>
      </div>
